        ...
        private void LateUpdate() {
            ...
            for (int i = 0; i <= rayCount; i++) {
                ...
                if (raycastHit2D.collider == null) {
                    ...
                    // Hit object
                    // Debug.Log("COLLIDING with -> " + raycastHit2D.collider);
                    if (raycastHit2D.collider.TryGetComponent(out Collectable collectable))
                    {
                        // Debug.Log("HIT AND GOT COLLECTABLE -> ", collectable);
                        collectable.EnableRenderer(true);
                    }
                    // Collectable collectable = raycastHit2D.collider.GetComponent<Collectable>();
                    // Debug.Log("Hit a collectable? -> " + collectable);
                    // Debug.Log("not null -> " + collectable != null);
